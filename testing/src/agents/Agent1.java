...
public class Agent1 implements Agent{
  ...
  class Node{
  	int id;
    State state;
    int score; // 0 for lose, 1 for win, -1 for unchecked
    List<Node> children;
    Node parent;

    Node(int id, State state, int score, List<Node> children, Node parent){
      this.id = id;
      this.state = state;
      this.score = score;
  ...
  public Node newNode(int id, State state, int score, List<Node> children, Node parent){
  	Node x = new Node(nodeCounter, state, score, children, parent);
  	nodeCounter++;
  	if (x.parent != null){
  ...
  public void newRound(State start){
    ...
    nodeCounter = 0;
    root = newNode(nodeCounter, current, -1, Collections.<Node>emptyList(), null);
  }
      ...
      //System.out.println("theres a GUARD in our hand");
    	...
    	Node parent = root;
      	...
      	for(int i = 0; i < possibleActions.size(); i++){
	        ...
	        Node child = newNode(nodeCounter, clone, -1, Collections.<Node>emptyList(), parent);
        
    	    while(clone.roundWinner() == -1){
        		// KEEP PLAYING GAME
        		// KEEP A COUNTER RUNNING?
        	}

        	if (clone.roundWinner() == myIndex){
        		child.score = 1;
        		return possibleActions.get(i); // if win return this action
        		// for now return immediately if win, if with counter we count which has least steps to win and return that action (?)
        	}
        	else {
	        	child.score = 0;
        	}
